...
public class ReminderEditActivity extends AppCompatActivity implements
    ...
    private Switch mAlarmstatSwitch;
...
//        mFAB2 = (FloatingActionButton) findViewById(R.id.starred2);
        ...
        mAlarmstatSwitch = (Switch) findViewById(R.id.alarmstat_switch);

        // Setup Toolbar
        setSupportActionBar(mToolbar);
        getSupportActionBar().setTitle("編輯提醒");
        ...
        mAlarmtype = mReceivedReminder.getAlarmtype();
        mAlarmstat = mReceivedReminder.getAlarmstat();
        ...
        if(mAlarmstat=="true"){ mAlarmstatText.setText("On");}
        else{ mAlarmstatText.setText("Off");}
...
//        }
        ...
        if (mAlarmstat.equals("false")) {
            mAlarmstatSwitch.setChecked(false);
            mAlarmstatText.setText(R.string.alarmstat_off);

        } else if (mAlarmstat.equals("true")) {
            mAlarmstatSwitch.setChecked(true);
...
//        mDate = dayOfMonth + "/" + monthOfYear + "/" + year;
    ...
    // On clicking the active button 註解或刪掉
...
//    }
    ...
    // On clicking the inactive button 註解或刪掉
...
//    }
    ...
    // On clicking alarmtype type button
    ...
    public void updateReminder(){
        ...
        mReceivedReminder.setAlarmtype(mAlarmtype);
        mReceivedReminder.setAlarmstat(mAlarmstat);
        ...
        // Create a new notification 改成alarmstat
...
//        }
        ...
        if (mAlarmstat.equals("true")) {
            if (mRepeat.equals("true")) {
                Log.e("重複鬧鐘","mCalendar"+mCalendar);
                mAlarmReceiver.setRepeatAlarm(getApplicationContext(), mCalendar, mReceivedID, mRepeatTime);
            } else if (mRepeat.equals("false")) {
                Log.e("無重複鬧鐘","mCalendar"+mCalendar);
                mAlarmReceiver.setAlarm(getApplicationContext(), mCalendar, mReceivedID);
            }
        }else{Log.e("關閉鬧鐘","mCalendar"+mCalendar);}


        // Create toast to confirm update
        Toast.makeText(getApplicationContext(), "已編輯",
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        switch (item.getItemId()) {
            ...
            case android.R.id.home:
                Toast.makeText(getApplicationContext(), "編輯已取消",
                        Toast.LENGTH_SHORT).show();
                onBackPressed();
                return true;
